...
export default function RedAlertOverlay() {
  ...
  const handleAck = async () => {
    ...
    try { await api.acknowledge(alert._id, { authorityId: 'GSDMA' }) } catch {}
  }
  const handleResolve = async () => {
    try { await api.resolve(alert._id) } catch {}
    clearTier3()
  }
  const handleOpen = () => {
        ...
        >
          ...
          >
            ...
            <div className="flex gap-3">
              <button
                onClick={handleAck}
                className="flex-1 py-4 rounded-xl bg-amber-500/25 border-2 border-amber-500/60 text-amber-200 font-black tracking-wide hover:bg-amber-500/40 transition-colors"
              >
                🛡 ACKNOWLEDGE (GSDMA)
              </button>
              <button
                onClick={handleResolve}
                className="flex-1 py-4 rounded-xl bg-emerald-500/25 border-2 border-emerald-500/60 text-emerald-200 font-black tracking-wide hover:bg-emerald-500/40 transition-colors"
              >
                ✓ RESOLVE INCIDENT
              </button>
            </div>
