...
export class TickScheduler {
  ...
  async start() {
    ...
    // Bootstrap history from dataset so graphs are full immediately
    await this.bootstrap(150); // 150 mins to ensure 120 window is fully saturated
  ...
  async _tickOneSite(site, tickCount) {
    ...
    if (tickCount % FORECAST_EVERY_N_TICKS === 0 || forecast.length === 0) {
      try {
        ...
        const fc = await mlForecast({ siteId: site.id, window, horizonMinutes: 5 });
  ...
  /** Pre-populate SensorHistory database with past data from the CSV. */
  async bootstrap(windowMinutes = 120) {
    console.log(`[ticks] bootstrapping ${windowMinutes} mins of history from dataset...`);
    const now = Date.now();
    const batch = [];

    for (const site of SITES) {
      const pastEnriched = simulator.bootstrapHistory(site.id, windowMinutes);
      // Map to DB rows with staggered timestamps in the past
      pastEnriched.forEach((r, i) => {
        const offset = (pastEnriched.length - 1 - i) * 60_000;
        batch.push({
          siteId: site.id,
          timestamp: new Date(now - offset),
          pressure: r.pressure_index,
          entry:    r.entry_flow_rate_pax_per_min,
          exit:     r.exit_flow_rate_pax_per_min,
          density:  r.queue_density_pax_per_m2,
          vehicles: r.vehicle_count,
          riskLevel: 'Low',
          confidence: 70,
          futurePressureT10: r.pressure_index,
          forecast15: [],
        });
      });
    }
    ...
    if (batch.length > 0) {
      try {
        ...
        console.log(`[ticks] bootstrapped ${batch.length} historical rows.`);
      } catch (err) {
        console.error(`[ticks] bootstrap failed: ${err.message}`);
      }
    }
  }
