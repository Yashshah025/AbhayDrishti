...
export class MultiSiteSimulator {
  ...
  /** Build the LSTM forecast window from the rolling history. */
  ...
  /** Pre-fill history by enriching N rows before the current cursor. */
  bootstrapHistory(siteId, count = 120) {
    const data = this.sitesData[siteId];
    if (!data || data.length === 0) return [];

    const currentIdx = this.cursor[siteId] ?? 0;
    const startIdx = Math.max(0, currentIdx - count);
    const bootRows = data.slice(startIdx, currentIdx);

    const enrichedSet = [];
    for (const raw of bootRows) {
      const enriched = this._enrich(siteId, raw);
      this.history[siteId].push(enriched);
      if (this.history[siteId].length > HISTORY_WINDOW) {
        this.history[siteId].shift();
      }
      enrichedSet.push(enriched);
    }
    return enrichedSet;
  }
