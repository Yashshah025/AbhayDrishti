...
async function main() {
  ...
  if (!process.env.MONGO_URL) {
    console.log('[mongo] MONGO_URL not set, starting in-memory MongoDB...');
    try {
      const mongod = await MongoMemoryServer.create();
      mongoUrl = mongod.getUri();
      console.log(`[mongo] in-memory MongoDB started at ${mongoUrl}`);
    } catch (err) {
      console.error('[mongo] Failed to start in-memory MongoDB:', err);
    }
  }
