...
export default function Header({ status, connected }) {
  ...
  return (
    <header className="relative z-10 glass-bright px-6 py-4 flex items-center justify-between mb-4 transition-all duration-300">
      {/* Logo + Title */}
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-xl font-black tracking-tight text-slate-900 dark:text-white text-glow-cyan transition-colors">
            CrowdShield
          </h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 leading-3 mt-0.5">
        ...
        {LOCATIONS.map(loc => (
          <span key={loc}
                className={`flex items-center gap-1 glass-dark px-3 py-1 rounded-full text-xs
                            ...
                            ${status?.location === loc
                              ? 'text-cyan-600 dark:text-cyan-300 border border-cyan-500/40 glow-cyan'
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2 text-xs">
          <Wifi className={`w-4 h-4 ${connected ? 'text-green-500 dark:text-green-400' : 'text-red-500 dark:text-red-400'}`} />
        ...
        {/* Theme Toggle */}
        <div className="border-l border-slate-200 dark:border-white/10 pl-4 ml-1">
          <ThemeToggle />
        </div>
      </div>
    </header>
